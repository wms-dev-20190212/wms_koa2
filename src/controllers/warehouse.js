const service = require('../service/warehouse');
const models = require('../models/warehouse'); //引入数据模型，可自动生成对应数据库表 gyx.sync({force: true});
...
module.exports = {
  'GET /api/warehouseAllList': async (ctx, next) => {
    await next();
    let sqlData
  ...
  'POST /api/warehouseList': async (ctx, next) => {
    try {
      await next();
  ...
  'PUT /api/warehouseList': async (ctx, next) => {
    try {
      await next();
  ...
  'DELETE /api/warehouseList/:id': async (ctx, next) => {
    try {
      await next();
