const service = require('../service/inoroutType');
const models = require('../models/inoroutType'); //引入数据模型，可自动生成对应数据库表 gyx.sync({force: true});
...
module.exports = {
  'GET /api/inoroutTypeAllList': async (ctx, next) => {
    await next();
    let sqlData
    sqlData = " select  distinct  a.id, a.name, a.fuzeren, a.type, a.isCost" +
      " from  inoroutType a  " +
      " where a.isDelete = 0  " +
      " ORDER BY a.id  ASC"
  ...
  'GET /api/inoroutTypeList': async (ctx, next) => {
    await next();
    let sqlData
    sqlData = " select  distinct  a.id, a.name, a.fuzeren, a.type, a.isCost" +
      " from  inoroutType a  " +
  ...
  'POST /api/inoroutTypeList': async (ctx, next) => {
    try {
      await next();
  ...
  'PUT /api/inoroutTypeList': async (ctx, next) => {
    try {
      await next();
  ...
  'DELETE /api/inoroutTypeList/:id': async (ctx, next) => {
    try {
      await next();
