...
const models = require('../models/company'); //引入数据模型，可自动生成对应数据库表 gyx.sync({force: true});
...
module.exports = {
  ...
  'POST /api/companyList': async (ctx, next) => {
    try {
      await next();
  ...
  'PUT /api/companyList': async (ctx, next) => {
    try {
      await next();
  ...
  'DELETE /api/companyList/:id': async (ctx, next) => {
    try {
      await next();
